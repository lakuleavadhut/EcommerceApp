...
const Sidebar = () => {
  ...
  return(
      ...
      md:w-[35vw] xl:max-w-[30vw] transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
        ...
        <div className='flex items-center justify-between py6 border-b'>
          <div className='uppercase text-sm font-semibold'>Shooping Bag (0)</div>
          <div onClick = {handleClose} className='cursor-pointer w-8 h-8 flex justify-center items-center'>
            <IoMdArrowForward className='text-2xl'/>
          </div>
        </div>
        <div className='flex flex-col gap-y-2 h-[480px] lg:h-[480px] overflow-y-auto overflow-x-hidden border-b'>
          {cart.map((item)=>{
        ...
        <div className='flex flex-col gap-y-3 py-4 mt-4'>
          ...
          <Link to = '/confirm' className="bg-primary flex p-4 justify-center items-center text-white w-full font-medium">checkout</Link>
        </div>
      </div>
      ); 
};
